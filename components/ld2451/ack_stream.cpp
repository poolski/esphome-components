...
size_t find_header(const std::vector<uint8_t> &rx_bytes, const uint8_t *header) {
  if (rx_bytes.size() < 4) {
...
void retain_header_prefix(std::vector<uint8_t> &rx_bytes) {
  if (rx_bytes.size() > 3) {
...
AckScanResult scan_for_matching_ack(std::vector<uint8_t> &rx_bytes, uint16_t command, uint16_t max_ack_payload_len,
  ...
  while (true) {
    const size_t data_pos = find_header(rx_bytes, DATA_HEADER);
    const size_t cmd_pos = find_header(rx_bytes, CMD_HEADER);

    if (data_pos == rx_bytes.size() && cmd_pos == rx_bytes.size()) {
      retain_header_prefix(rx_bytes);
    ...
    const uint8_t *expected_tail = next_is_data ? DATA_TAIL : CMD_TAIL;
